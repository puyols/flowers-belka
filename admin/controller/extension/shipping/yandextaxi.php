...
use YandexTaxi\Services\CountryRelatedDataService;
...
class ControllerExtensionShippingYandexTaxi extends Controller {
    ...
    private function init(): void {
        ...
        $this->document->addScript('view/javascript/yandextaxi/lib/jquery.inputmask.min.js?v=' . Constants::VERSION);
    ...
    public function index() {
        ...
        if ($this->responds->isPost() && $this->validateSettings($this->request->post)) {
            ...
            try {
                ...
                if (!(new AvailableTariffChecker(new Tariffs($api)))->isAvailableDefault($this->setting_service)) {
                ...
                $message = $this->responds->output(
                    'partial/_bad_token',
                    ['connectUrlSource' => json_encode(CountryRelatedDataService::getConnectUrlSource())]
                );
        ...
        $this->responds->view('yandextaxi_settings_form', array_merge([
            ...
            'geocoderSource' => json_encode(CountryRelatedDataService::getGeocoderInstructionUrlSource()),
            'connectUrlSource' => json_encode(CountryRelatedDataService::getConnectUrlSource()),
            'cabinetUrlSource' => json_encode(CountryRelatedDataService::getCabinetUrlSource()),
            'vat_codes' => [
                'vat_none' => $this->language->get('vat_none'),
                'vat0' => $this->language->get('vat_0'),
                'vat10' => $this->language->get('vat_10'),
                'vat20' => $this->language->get('vat_20'),
            ],
        ], $settings));
    }
    ...
    public function createOrder() {
        ...
        $this->responds->view('yandextaxi_create_order', array_merge([
            ...
            'phone_country' => CountryRelatedDataService::getPhoneCountry($this->setting_service->getOne('shipping_yandextaxi_country') ?? '')
    ...
    public function createClaim() {
        ...
        try {
            $this->shipment_service->createClaimForCalculation([
                ...
                'orders' => $this->order_repository->getOrdersByIds($order_ids),
                'products' => $this->order_repository->getOrdersProductsByOrderIds($order_ids, $this->config->get('config_language_id')),
                'shipping' => $this->order_repository->getOrderShippingByOrderIds($order_ids),
    ...
    public function getPrice() {
        ...
        if (is_null($claim->getPrice())) {
            ...
            $response = [
                'calculated' => true,
                'price' => round($claim->getPrice()->getValue(), 2) . CountryRelatedDataService::getCurrencySymbol($claim->getPrice()->getCurrency()),
    ...
    public function getShipment() {
        ...
        $this->responds->view('partial/_order_shipping_info', [
            'shipment' => (new ShipmentPresenter(
                $this->language,
                $this->setting_service,
                $this->url,
                $this->session->data['user_token'])
            )->present($shipment, $order_id),
        ]);
    }
    ...
    private function getDestinationsHtml(array $orders): array {
        ...
        $country = $this->setting_service->getOne('shipping_yandextaxi_country') ?? '';
        $inn = trim($this->setting_service->getOne('shipping_yandextaxi_supplier_inn'));
        $isPaymentOnDeliveryAllowed = CountryRelatedDataService::isPaymentOnDeliveryAllowed($country) && !empty($inn);
        ...
        foreach ($orders as $i => $order) {
            $destinations[] = $this->responds->output('partial/_destination', [
                    ...
                    'destination' => [
                        ...
                        'payment_on_delivery_cash' => $order['payment_code'] === 'cod' && $isPaymentOnDeliveryAllowed,
                        'payment_on_delivery_card' => $order['payment_code'] === 'yandexgodelivery_card_od' && $isPaymentOnDeliveryAllowed,
