...
class ControllerCommonFileManager extends Controller {
	...
	public function upload() {
		...
		if (!$json) {
			...
			require_once(DIR_OPENCART . 'includes/ImageOptimizer.php');
			...
			foreach ($files as $file) {
				...
				if (!$json) {
					...
					// Optimize the image and create WebP versions
					try {
						$results = $optimizer->optimizeImage($temp_path, $directory, pathinfo($filename, PATHINFO_FILENAME));
						...
						if (!empty($results)) {
							...
							foreach ($results as $size => $formats) {
								if (isset($formats['webp']) && file_exists($formats['webp'])) {
									$webp_size = filesize($formats['webp']);
									$optimized_info[] = $size . ' WebP: ' . ImageOptimizer::formatFileSize($webp_size);
								}
							}

							if (!empty($optimized_info)) {
								$json['optimization'] = 'Оригинал: ' . ImageOptimizer::formatFileSize($original_size) .
									'. Создано: ' . implode(', ', $optimized_info);
							}
						}
					} catch (Exception $e) {
						// If optimization fails, keep the original file
						$json['warning'] = 'Изображение загружено, но оптимизация не удалась: ' . $e->getMessage();
					}
				}
			}
		}
