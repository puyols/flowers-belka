...
class ModelExtensionShippingYandexTaxi extends Model {
    ...
    function getQuote($address) {
        ...
        $settingGeoZoneId = $this->config->get('shipping_yandextaxi_geo_zone_id');

        if (!empty($settingGeoZoneId)) {
            $query = $this->db->query(
                "SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone "
                . "WHERE geo_zone_id = '" . (int)$settingGeoZoneId
        ...
        try {
            $price = $this->client_service->calculateSum(
