...
class ControllerExtensionFeedYandexMarket extends Controller {
	...
	public function index() {
		if ($this->config->get('feed_yandex_market_status')) {
			...
			foreach ($products as $product) {
				...
				$data['url'] = $this->url->link('product/product', 'path=' . $this->getPath($product['category_id']) . '&product_id=' . $product['product_id'], true);
				if ($product['price']>0) {
	...
	private function setOffer($data) {
		...
		$allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'oldprice'=>0, 'sales_notes'=>0, 'wprice'=>0, 'currencyId'=>1, 'xCategory'=>0, 'categoryId'=>1, 'pictures'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'instock'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0);
	...
	private function array2Tag($tags) {
		...
		foreach ($tags as $key => $value) {
			if ($key == 'pictures'){
				...
				$retval .= '<outlets><outlet id="1" instock="' . $value . '"/></outlets>' . $this->eol;
			} else {
