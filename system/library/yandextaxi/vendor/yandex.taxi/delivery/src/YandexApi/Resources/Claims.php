...
class Claims extends Resource
{
    ...
    private function claimToArray(Claim $claim): array
    {
        ...
        foreach ($claim->getItems() as $item) {
            ...
            if ($item->getFiscalization() !== null) {
                $itemArray['fiscalization'] = [
                    'article' => $item->getFiscalization()->getArticle(),
                    'supplier_inn' => $item->getFiscalization()->getSupplierInn(),
                    'vat_code_str' => $item->getFiscalization()->getVatCode(),
                ];
            }
    ...
    private function prepareRoutePoint(RoutePoint $point, int $visitOrder, string $type): array
    {
        ...
        if (!is_null($point->getExternalOrderCost())) {
            $params['external_order_cost'] = [
                'value' => $point->getExternalOrderCost()->getValue(),
                'currency' => $point->getExternalOrderCost()->getCurrency(),
                'currency_sign' => '',
            ];
        }

        if (!is_null($point->getPaymentOnDelivery())) {
            $params['payment_on_delivery']['payment_method'] = $point->getPaymentOnDelivery()->getPaymentMethod();

            if (!empty($point->getPaymentOnDelivery()->getCustomerEmail())) {
                $params['payment_on_delivery']['customer'] = [
                    'email' => $point->getPaymentOnDelivery()->getCustomerEmail()
                ];
            }
        }
